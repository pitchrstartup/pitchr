...
function inferPayloadType(fieldName) {
  ...
  if (fieldName === 'rawListPayload') return 'Json';
  if (fieldName === 'rawDetailPayload') return 'Json';
  if (fieldName === 'upvotes' || fieldName === 'downvotes') return 'Int';
  if (fieldName === 'twitterVerified') return 'Boolean';
  return 'String';
}
...
export function normalizeProject(project, index) {
  ...
  const twitterUser = detail.twitterUser && typeof detail.twitterUser === 'object' ? detail.twitterUser : null;

...
async function persistCursor({ prisma, cursor, batchSize, totalProjects, projectsProcessed }) {
  ...
  for (const project of projects) {
    ...
    try {
      while (totalItems === null || offset < totalItems) {
        ...
        for (const updatePayload of page.updates) {
          const normalized = normalizeBagsUpdate({ project, update: updatePayload });
          if (!normalized.ok) {
            rejectedRows.push({
              sourceProjectUuid: project.sourceProjectId,
